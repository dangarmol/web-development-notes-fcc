...
mongoose.connect(process.env.MONGO_URI, { useNewUrlParser: true, useUnifiedTopology: true });  // The key to connect is stored as an environment variable on .env. Obviously, never commit such a file to Git, only done for educational purposes.
...
var createAndSavePerson = function(done) {
  ...
  let myPerson = Person(personObject);
...
// let personId = "1a2b3c4d5e";  // Already declared on previous exercise.

var findEditThenSave = function(personId, done) {
  var foodToAdd = "hamburger";
  
  Person.findById(personId, function (err, originalPerson) {
    if (err) return console.log(err);

    originalPerson.favoriteFoods.push(foodToAdd);

    originalPerson.save(function(err, modifiedPerson) {
      if (err) return console.error(err);
      done(null, modifiedPerson);
    });
  });
...
var findAndUpdate = function(personName, done) {
  ...
  Person.findOneAndUpdate(personId, { "age": ageToSet }, { new: true }, function (err, data) {
    if (err) return console.log(err);
    done(null, data);
  });
...
var queryChain = function(done) {
  ...
  Person.find({ "favoriteFoods": foodToSearch }).sort("name").limit(2).select("name favouriteFoods").exec(function (err, data) {
    if (err) return console.log(err);
    done(null, data);
  });
};

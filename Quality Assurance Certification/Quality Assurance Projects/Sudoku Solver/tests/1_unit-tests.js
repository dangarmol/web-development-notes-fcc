...
suite('UnitTests', () => {
  ...
  suite('Function validate()', () => {
    test('Valid Characters, length of 81', (done) => {
      ...
      assert.isTrue(solver.validatePuzzleLength(input));
      done();
    });
    ...
    test('Invalid characters (anything other than "1-9" or "."") are not accepted', (done) => {
      ...
      assert.isFalse(solver.validatePuzzleCharacters(input));
      done();
    });
    ...
    test('Shows an error for puzzles that are not 81 numbers long', done => {
      ...
      assert.isFalse(solver.validatePuzzleLength(shortStr));
      assert.isFalse(solver.validatePuzzleLength(longStr));
      done();
    });
  });
  ...
  suite('Function checkRowPlacement()', () => {
    test('Valid placement for a row', done => {
      ...
      const col = 0;
      const value = 3;
      assert.isTrue(solver.checkRowPlacement(input.split(""), row, value));
      done();
    });
    ...
    test('Invalid placement for a row', done => {
      ...
      const col = 0;
      const value = 9;
      assert.isFalse(solver.checkRowPlacement(input.split(""), row, value));
      done();
    });
  ...
  suite('Function checkColPlacement()', () => {
    test('Valid placement for a column', done => {
      ...
      const col = 0;
      const value = 3;
      assert.isTrue(solver.checkColPlacement(input.split(""), col, value));
      done();
    });
    ...
    test('Invalid placement for a column', done => {
      ...
      const col = 0;
      const value = 8;
      assert.isFalse(solver.checkColPlacement(input.split(""), col, value));
      done();
    });
  ...
  suite('Function checkRegionPlacement()', () => {
    test('Valid placement for a region', done => {
      ...
      const col = 0;
      const value = 7;
      assert.isTrue(solver.checkRegionPlacement(input.split(""), row, col, value));
      done();
    });
    ...
    test('Invalid placement for a region', done => {
      ...
      const col = 4;
      const value = 7;
      assert.isFalse(solver.checkRegionPlacement(input.split(""), row, col, value));
      done();
    });
  ...
  suite('Function solvePuzzle()', () => {
    ...
    test('Valid puzzles pass', done => {
      const input = '..9..5.1.85.4....2432......1...69.83.9.....6.62.71...9......1945....4.37.4.3..6..';
      assert.isString(solver.solveString(input));
      done();
    });
    ...
    test('Invalid puzzles fail', done => {
      ...
      assert.isFalse(solver.solveString(input));
      done();
    });
    ...
    test('Returns the expected solution for an incomplete puzzle', done => {
      ...
      assert.equal(solver.solveString(input), solution);
      done();
    });
  });
});
